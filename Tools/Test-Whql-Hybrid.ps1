...
param(
    ...
    [switch]$AllowLocalCatalogSigning,
    [switch]$SkipCatalogSigning
)

$ErrorActionPreference = 'Stop'
[Console]::OutputEncoding = New-Object Text.UTF8Encoding($false)
$profilePath = if ([IO.Path]::IsPathRooted($ProfilePath)) { $ProfilePath } else { Join-Path $ProjectRoot $ProfilePath }
...
$infName = $null
$kernelDriverPath = $null
$binaryFolder = $null
$originalCatalogName = $null
$prepareRoot = $null
$linkSoftwareRoot = $null
$link25Root = $null
$backupRoot = $null
$statePath = $null
$anchorVersion = '32.0.12033.5029'
...
function Initialize-ProfileContext($LoadedProfile) {
    $script:marketingVersion = [string]$LoadedProfile.marketingVersion
    $script:infName = [string]$LoadedProfile.infName
    $script:kernelDriverPath = ([string]$LoadedProfile.kernelDriverPath) -replace '\\', '/'
    $script:binaryFolder = (Split-Path -Parent $script:kernelDriverPath) -replace '\\', '/'
    $script:originalCatalogName = ($LoadedProfile.files | Where-Object { $_.path -like '*.cat' -and $_.path -ne 'amdgpu.cat' } | Select-Object -First 1).path
    if (-not $script:originalCatalogName) {
        $script:originalCatalogName = [string]$LoadedProfile.infName -replace '\.inf$', '.cat'
    }
    $slug = $script:marketingVersion -replace '\.', ''
    if (-not $ResultPath) {
        $script:ResultPath = "C:\AMD\whql-hybrid-$slug-25.2.1-result.txt"
    }
    $script:statePath = "C:\AMD\whql-hybrid-$slug-25.2.1-pending.json"
    $script:prepareRoot = "C:\AMD\BootCampDriverStudio\Prepared\AMD-$($LoadedProfile.marketingVersion)-with-25.2.1-kernel-$stamp"
    $script:linkSoftwareRoot = "C:\AMD\BootCampDriverStudio\Prepared\WHQL-Link-$($LoadedProfile.marketingVersion)-$stamp"
    $script:link25Root = "C:\AMD\BootCampDriverStudio\Prepared\WHQL-Link-25.2.1-hybrid-$stamp"
    $script:backupRoot = "C:\ProgramData\AMD BootCamp Driver Studio\Backups\before-whql-hybrid-$slug-$stamp"
}
...
function Get-ProfileHashMap($Profile) {
    ...
    $infRule = $Profile.files | Where-Object { $_.path -eq $infName } | Select-Object -First 1
    if ($infRule -and $infRule.patchedSha256) {
        $map["patched:$infName"] = [string]$infRule.patchedSha256
    }
    return $map
}
...
function Test-TestSigningActive {
    ...
    return (($startOptions -split '\s+') -contains 'TESTSIGNING')
}

function Assert-HybridInstallAllowed([string]$PackageRoot, $Profile) {
    if ($PrepareOnly -or $AllowLocalCatalogSigning -or (Test-TestSigningActive)) { return }
    if ($MsSignedCatalogPath) { return }
    $catalog = Join-Path $PackageRoot ([string]$Profile.catalogFile)
    if (-not (Test-Path -LiteralPath $catalog)) { throw "Catalog missing before install policy check: $catalog" }
    $sig = Get-AuthenticodeSignature -LiteralPath $catalog
    $localSubject = [string]$Profile.certificateSubject
    if ($sig.Status -eq 'Valid' -and $sig.SignerCertificate.Subject -eq $localSubject) {
        throw 'Local catalog signing is blocked while TESTSIGNING is OFF. Use -MsSignedCatalogPath with a Microsoft-trusted catalog, or pass -AllowLocalCatalogSigning for experimental installs only.'
    }
    if ($sig.SignerCertificate.Subject -notlike 'CN=Microsoft Windows Hardware Compatibility Publisher*') {
        throw "Hybrid install requires a Microsoft-trusted catalog while TESTSIGNING is OFF. Actual signer=$($sig.SignerCertificate.Subject)"
    }
    Add-Result "INSTALL_POLICY_OK signer=$($sig.SignerCertificate.Subject)"
}
...
function New-WhqlLink([string]$SourceRoot, [string]$InfName, [string]$CatName, [string]$BinaryFolderName, [string]$Destination) {
    New-Item -ItemType Directory -Path (Join-Path $Destination $BinaryFolderName) -Force | Out-Null
    ...
    foreach ($file in Get-ChildItem -LiteralPath (Join-Path $SourceRoot $BinaryFolderName) -File) {
        New-Item -ItemType File -Path (Join-Path $Destination ($BinaryFolderName + '\' + $file.Name)) -Force | Out-Null
    }
    $placeholders = Get-ChildItem -LiteralPath (Join-Path $Destination $BinaryFolderName) -File
...
function Restore-Backup($Profile, [string]$Id) {
    ...
    if ($gpu) {
        ...
        return ($code -eq 0 -and $version -eq $anchorVersion)
    }
    return $false
}
...
function Save-PendingState {
    [pscustomobject]@{
        ...
        LinkSoftwareRoot = $linkSoftwareRoot
        ...
        MsSignedCatalogPath = $MsSignedCatalogPath
...
function Prepare-HybridPackage($Profile, $Hashes) {
    $kernelDest = [string]$Profile.sourceFileCopies[0].destinationPath
    $kernelSourcePath = [string]$Profile.sourceFileCopies[0].sourcePath
    $kernelSourceId = [string]$Profile.sourceFileCopies[0].sourceId

    Assert-Hash (Join-Path $SoftwareSourceRoot $infName) $Hashes[$infName] "$marketingVersion INF"
    Assert-Hash (Join-Path $SoftwareSourceRoot $originalCatalogName) $Hashes[$originalCatalogName] "$marketingVersion CAT"
    Assert-Hash (Join-Path $SoftwareSourceRoot $kernelDriverPath) $Hashes[$kernelDriverPath] "$marketingVersion kernel"
    if ($Hashes.ContainsKey("$binaryFolder/amdgcf.dat")) {
        Assert-Hash (Join-Path $SoftwareSourceRoot "$binaryFolder/amdgcf.dat") $Hashes["$binaryFolder/amdgcf.dat"] "$marketingVersion amdgcf.dat"
    }
    Assert-Hash (Join-Path $KernelSourceRoot 'u0412654.inf') $Hashes["$kernelSourceId/u0412654.inf"] '25.2.1 INF'
    Assert-Hash (Join-Path $KernelSourceRoot 'u0412654.cat') $Hashes["$kernelSourceId/u0412654.cat"] '25.2.1 CAT'
    Assert-Hash (Join-Path $KernelSourceRoot $kernelSourcePath) $Hashes["copy:$kernelDest"] '25.2.1 kernel'

    foreach ($catalog in @((Join-Path $SoftwareSourceRoot $originalCatalogName), (Join-Path $KernelSourceRoot 'u0412654.cat'))) {
        $sig = Get-AuthenticodeSignature -LiteralPath $catalog
    ...
    $kernelSource = Join-Path $KernelSourceRoot ($kernelSourcePath -replace '/', '\')
    ...
    Copy-Item -LiteralPath $kernelSource -Destination (Join-Path $prepareRoot ($kernelDest -replace '/', '\')) -Force
    Add-Result "HYBRID_PACKAGE_COPIED=$prepareRoot"
    Add-Result "ORIGINAL_KERNEL_SUBSTITUTED 25.2.1=$($Hashes["copy:$kernelDest"])"

    foreach ($patch in $Profile.patches) { Apply-TextPatch $patch $prepareRoot }
    $hybridInf = Join-Path $prepareRoot $infName
    Assert-Hash $hybridInf $Hashes["patched:$infName"] 'hybrid INF'

    $skip = @($infName, ($kernelDest -replace '/', '\'))
    $unchanged = 0
    foreach ($source in Get-ChildItem -LiteralPath $SoftwareSourceRoot -Recurse -File) {
        ...
        if ($sourceHash -ne $copyHash) { throw "$marketingVersion binary changed: $relative" }
        $unchanged++
    }
    ...
    Add-Result "ALL_SOFTWARE_NON_KERNEL_FILES_UNCHANGED count=$unchanged"

    if ($MsSignedCatalogPath) {
        if (-not (Test-Path -LiteralPath $MsSignedCatalogPath)) { throw "Microsoft-signed catalog missing: $MsSignedCatalogPath" }
        $destCatalog = Join-Path $prepareRoot ([string]$Profile.catalogFile)
        Copy-Item -LiteralPath $MsSignedCatalogPath -Destination $destCatalog -Force
        $msSig = Get-AuthenticodeSignature -LiteralPath $destCatalog
        if ($msSig.Status -ne 'Valid' -or $msSig.SignerCertificate.Subject -notlike 'CN=Microsoft Windows Hardware Compatibility Publisher*') {
            throw "Microsoft-signed catalog validation failed: $destCatalog ($($msSig.Status))"
        }
        Add-Result "MS_TRUSTED_CATALOG_APPLIED=$destCatalog"
    }
    elseif ($SkipCatalogSigning) {
        Add-Result 'CATALOG_SIGNING_SKIPPED attestation_export'
    }
    else {
        & $signScript -PackageRoot $prepareRoot -KernelDriverPath $kernelDriverPath `
            -CatalogFile ([string]$Profile.catalogFile) -CertificateSubject ([string]$Profile.certificateSubject) -SkipKernelSigning
        Add-Result 'LOCAL_HYBRID_CATALOG_SIGNED'
        & $signScript -PackageRoot $prepareRoot -KernelDriverPath $kernelDriverPath `
            -CatalogFile ([string]$Profile.catalogFile) -CertificateSubject ([string]$Profile.certificateSubject) -SkipKernelSigning -ValidateOnly
...
function Assert-HybridRuntime($Profile, $Hashes, [string]$ExpectedVersion) {
    ...
    foreach ($assertion in $Profile.runtimeFileAssertions) {
        if ($assertion.path -like '*/amdkmdag.sys') {
            Assert-Hash $loadedKernel $Hashes["runtime:$([string]$assertion.path)"] 'loaded hybrid kernel'
            $activeRepo = Split-Path -Parent (Split-Path -Parent $loadedKernel)
            $folderName = Split-Path -Parent ($assertion.path -replace '/', '\')
            foreach ($other in $Profile.runtimeFileAssertions | Where-Object { $_.path -notlike '*/amdkmdag.sys' }) {
                $otherPath = Join-Path $activeRepo (($other.path -replace '/', '\') -replace "^$folderName\\", "$folderName\")
                $label = "loaded $($other.path)"
                Assert-Hash $otherPath $Hashes["runtime:$([string]$other.path)"] $label
            }
            break
        }
    }
    Add-Result "HYBRID_COMPONENTS_OK kernel=25.2.1 user_mode=$marketingVersion service=$service"
}

try {
    $profile = Load-Profile $profilePath
    Initialize-ProfileContext $profile
    ...
    Add-Result "PROFILE=$($profile.id) VERSION=$expectedVersion"
    ...
    if ($ResumeAfterReboot) {
        ...
        $linkSoftwareRoot = [string]$state.LinkSoftwareRoot
        if ($state.Link26Root) { $linkSoftwareRoot = [string]$state.Link26Root }
        ...
        if ($state.ProfilePath) { $profilePath = [string]$state.ProfilePath; $profile = Load-Profile $profilePath; Initialize-ProfileContext $profile; $hashes = Get-ProfileHashMap $profile; $expectedVersion = [string]$profile.driverVersion }
        ...
        if ($state.MsSignedCatalogPath) { $MsSignedCatalogPath = [string]$state.MsSignedCatalogPath }
        $driverChanged = $true
        Add-Result "RESUME_AFTER_REBOOT prepared=$prepareRoot"

        if (Test-TestSigningActive) { throw 'TESTSIGNING is active after reboot.' }
        Assert-HybridRuntime $profile $hashes $expectedVersion
        $gpu = Get-Gpu $hardwareId
        Apply-RegistrySettings $profile $gpu

        if ($RunWhqlDiagnostics) {
            New-WhqlLink $SoftwareSourceRoot $infName $originalCatalogName $binaryFolder $linkSoftwareRoot
            New-WhqlLink $KernelSourceRoot 'u0412654.inf' 'u0412654.cat' 'B412641' $link25Root
            foreach ($linkInf in @((Join-Path $link25Root 'u0412654.inf'), (Join-Path $linkSoftwareRoot $infName))) {
            ...
            Invoke-WhqlDiagnostics "C:\AMD\dxdiag-whql-hybrid-$marketingVersion-25.2.1-$stamp.txt" | Out-Null
        }
    ...
    if (-not $PrepareOnly -and (Test-TestSigningActive)) { throw 'TESTSIGNING is active.' }
    ...
    Assert-HybridInstallAllowed $prepareRoot $profile
    ...
    if ($initialCode -ne 0 -or $initialVersion -ne $anchorVersion) {
    ...
    if (-not (Test-Path -LiteralPath $hybridInf)) { throw "Hybrid INF missing before install: $hybridInf" }
    Add-Result "HYBRID_INSTALL_INF=$hybridInf"
    ...
    if ($TestRollbackAfterInstall) { throw 'Intentional rollback test after hybrid installation.' }
    ...
    if (Test-TestSigningActive) { throw 'TESTSIGNING unexpectedly became active.' }
    Add-Result "FINAL_TESTSIGNING_OFF SystemStartOptions=$finalStartOptions"
    Add-Result "PREPARED_ROOT=$prepareRoot"
    Add-Result "WHQL_LINK_SOFTWARE_ROOT=$linkSoftwareRoot"
